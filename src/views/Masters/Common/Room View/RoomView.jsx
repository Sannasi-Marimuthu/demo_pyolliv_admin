...
export default function PropertyType() {
  ...
  const columns = [
    { title: 'Room View', dataIndex: 'RoomView', key: 'RoomView' },
    {
      title: 'Inactive',
      dataIndex: 'Status',
      key: 'Status',
  ...
  const id = sessionStorage.getItem('Roomview')
  const handlesubmit = () => {
    try {
      if (!roomview) {
        return toast.error('Room View is required')
      }

      const RoomView = roomview?.toString().trim() || ''
      ...
      console.log(RoomViewData)

      const apiCall = id
        ? api.update(`update?id=${id}&type=Roomview`, RoomViewData)
        : api.create('post', RoomViewData)

      apiCall
        .then(() => {
          toast.success(id ? 'Updated Successfully' : 'Created Successfully')
          if (id) sessionStorage.removeItem('Roomview')
          navigate(0)
        })
        .catch((error) => {
          console.error(error)
  ...
  const editUser = (record) => {
    ...
    sessionStorage.setItem('Roomview', record._id)
  }
  ...
  return (
    <div>
      <div className="tableicon">
        {/* <h3 className="heading">Room View</h3> */}
        {newfile && (
          <h6
            onClick={() => {
              setViewForm(true)
            ...
            <div className="admin-row">
              <div className="admin-col">
                <div className="inputs">
                  <p className="responsive">Room View</p>
                  <Input
                    maxLength={30}
                    className="inputbox"
                    ...
                    value={roomview}
                    onChange={(e) => {
                      validation.sanitizeInput1(e.target.value, setRoomView)
                    }}
                  />
                </div>
              </div>
